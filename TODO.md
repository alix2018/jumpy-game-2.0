# TODO

## SETTINGS PAGE
- Improve layout on mobile/tablet
- [x] Fix asset quality (re-upload smaller idle assets ~80×307 for Luc and ~150×282 for Shannon)
- [x] Put bold titles
- [x] Disabled button more transparent
- [x] Keep disable text all the time
- [x] Save selected language
- Make gradient animation border fade in/out to highlight selection
- Choose another font

## GAME
- [x] Add settings icon
- [x] Settings icon should be dark gray
- [x] Falling asset has a lower impact than the other assets (arrives too low on the platforms)
- Add overlay to display the rules (logique: display the rules only once for each device, save the data in the local storage to know if the user already had read the rule. Display the rules with a black transparent overlay with rounded corners on top of the game after clicking "play", the game should not start it should be paused at the beginning by default. On the overlay use the title "rules", the text should be: "TODO: Add the rules here". And add a button at the bottom of the overlay saying "I understand, play now". Once the user clicks on this button the game starts from the beginning)

## END PAGE
- Save the date page
- Show the page if it matches a set of rules

## OTHERS
- Add metadata + favicon